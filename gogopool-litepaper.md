---
description: 'GoGoPool: An Open, Trustless Decentralized Staking Protocol'
---
...
We have two tokens: gAVAX (liquid staking token described above), and GGP (protocol token used for DAO governance, rewards, insurance, and incentivizing long term behaviour).

**gAVAX**

When a user deposits AVAX into the deposit pool, they receive a synthetic derivative token called gAVAX.

gAVAX represents a staker’s deposit plus the rewards it gains over time. This token is considered liquid and can be used in any way that AVAX is used. Users will be able to exchange gAVAX back for AVAX (which burns the gAVAX, and draws AVAX from the deposit pool). Alternatively, they will have the option to list it on any exchange listing the token and exchange it for any token they would like.
...
\- Beta launched on Fuji testnet w/o subnet compatibility

\- Q2

\- DAOs formed

\- OracleDAO members chosen\
\- New name chosen and white paper signed by founding members of DAOs

\- Subnet compatibility on Fuji

\- Mainnet launch w/o subnet compatibility

\- Begin work with Ava Labs on smart contract withdrawals

\-Q3+

\- Mainnet launch w/ subnet compatibility\
\- Scaling of network\
\- More governance features implemented

\- Lower cost of staking

To further the general web3 ecosystem, all code will be GPLV3 licensed.

**Acknowledgements**

Rocketpool.net, for being the first to create an open, decentralized staking protocol

Ava Labs, for creating a flexible, green and fast L0 and L1 solution

The GoGoPool Dev team, for building the dream\
Ethereum Foundation and BitCoin for laying the first pieces of a foundation for web3
